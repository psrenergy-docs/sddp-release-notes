...
* GUI
  * Deprecation of Graph 3 interface
...
* Model
  ...
  * The hydroelectric storage data now allows the definition of explicit limits for run-of-river plants, which are used in the chronological blocks, typical days and hourly representation for intra-stage modulation
    ...
    * Added option to allow automatic classification of hydroelectric plants based on its regularization capacity (useful storage and turbining capacity ratio)
  ...
  * Network contingency representation. This features includes an option to enable contigencies representation from a specific iteration in the policy
  * Post-contingency network report
  * Operational status of DC link
  * Custom modeling
    * Generic interpolation constraints for the modeling of non-linear functions
